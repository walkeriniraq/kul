...
describe Kul::Path do
  ...
  describe '#controller_name' do
    it 'returns the name' do
      Kul::Path.new('bar/baz?test=thing').controller_name.should == 'bar'
    end
    it 'returns name when it ends path with params' do
      test = Kul::Path.new('bar?test=thing').controller_name
      test.should == 'bar'
    end
    it 'returns name when it ends path' do
      test = Kul::Path.new('bar').controller_name
      test.should == 'bar'
    end
    ...
    it 'should return the same string' do
      request = Kul::Path.new('bar/baz?test=thing')
  ...
  describe '#action_name' do
    it 'returns the name' do
      test = Kul::Path.new('bar/baz?test=thing').action_name
      test.should == 'baz'
    end
    it 'returns index if no action' do
      test = Kul::Path.new('bar?test=thing').action_name
      test.should == 'index'
    end
    ...
    it 'returns nil when the path is longer than an action' do
      test = Kul::Path.new('bar/baz/more.html?test=thing').action_name
      test.should be_nil
    end
    it 'returns the name when there are no params' do
      test = Kul::Path.new('bar/baz').action_name
      test.should == 'baz'
    end
    it 'should return the same string' do
      request = Kul::Path.new('bar/baz?test=thing')
