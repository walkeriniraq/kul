...
describe Kul::RequestContext do
  ...
  describe '#route_to_file' do
    ...
    it 'returns template render if the template exists' do
      inside_test_server do
        request = Kul::RequestContext.new(path: 'app/context_test.html', verb: :GET)
    ...
    it 'returns compile render if the file exists' do
      inside_test_server do
        request = Kul::RequestContext.new(path: 'js/test_coffee.js', verb: :GET)
        test = request.route_to_file
        test.should be_a ResponseCompiledFile
        test.file.should == 'js/test_coffee.js.coffee'
      end
    end
  end
  ...
  describe '#render_action' do
    it 'returns the response from the action' do
      inside_test_server do
        request = Kul::RequestContext.new(path: 'bar/test_action', verb: :GET)
        request.render_action.should == 'this is a nifty test action'
      end
    end
    it 'passes the context to the action' do
      inside_test_server do
        request = Kul::RequestContext.new(path: 'bar/repeat_action', verb: :GET, params: { 'test' => 'a thing'})
        request.render_action.should == 'The test value passed was: a thing'
      end
    end
    it 'handles POST verbs' do
      inside_test_server do
        request = Kul::RequestContext.new(path: 'bar/post_action', verb: :POST, params: { 'test' => 'a thing'})
        request.render_action.should == 'The test value passed was: a thing'
      end
    end
  end
